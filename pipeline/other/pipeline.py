...
class ModelPipeline:
    ...
    def getImage(self):
        self.validateImage()
        return io.imread(os.getcwd().strip('other') + self.img_path)
    ...
    def modelPipeline(self):
        ...
        model.load_state_dict(torch.load(os.getcwd().strip('other') + self.model_path, map_location=torch.device('cpu')))
        model.eval()
    ...
    def showImage(self, pred_str):
        ...
        im = Image.open(os.getcwd().strip('other') + self.img_path)
        ax1.imshow(im)
